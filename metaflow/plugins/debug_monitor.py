...
class DebugMonitor(object):
    ...
    def measure(self, timer):
        # type: (Timer) -> None
        self.logger(f'elapsed time for {timer.name}: {str(timer.get_duration())}')
    ...
    def process_message(self, msg):
        # type: (Message) -> None
        self.logger(f'processing message {str(msg.msg_type)}')
    ...
    def logger(self, msg):
        print(f'local_monitor: {msg}', file=sys.stderr)
        sys.stderr.flush()
